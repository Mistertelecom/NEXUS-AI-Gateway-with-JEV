...
function getDbPath(): string | null {
  ...
  // Build the path dynamically at runtime so Turbopack's static AST asset
  // analyzer cannot trace .codegraph as a bundled filesystem asset reference.
  const root = process.cwd();
  const folder = [".", "code", "graph"].join("");
  const file = ["code", "graph", ".db"].join("");

  const p1 = join(root, folder, file);
  if (existsSync(p1)) return p1;

  const p2 = join(root, "..", folder, file);
