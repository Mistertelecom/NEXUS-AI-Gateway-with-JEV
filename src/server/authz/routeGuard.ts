...
export const LOCAL_ONLY_API_PREFIXES: ReadonlyArray<string> = [
  ...
  "/api/nexus/", // NEXUS control plane can spawn CLIs, write local config, and expose local runtime evidence
...
/**
 * LOCAL_ONLY paths that must never inherit the repository-wide private-LAN carve-out.
 * NEXUS can execute caller-selected commands and write host configuration, so its
 * management plane remains bound to the machine that owns the workspace.
 */
export const STRICT_LOOPBACK_API_PREFIXES: ReadonlyArray<string> = ["/api/nexus/"];
...
export const ALWAYS_PROTECTED_API_PATHS: ReadonlyArray<string> = [
  "/api/shutdown",
  // NEXUS can execute local commands and write client configuration. LAN callers
  // must authenticate even when the general dashboard login switch is disabled.
  "/api/nexus/",
...
export function isLocalOnlyPath(path: string, method?: string): boolean {
  if (isStrictLoopbackOnlyPath(path)) return true;
...
export function isStrictLoopbackOnlyPath(path: string): boolean {
  return STRICT_LOOPBACK_API_PREFIXES.some(
...
export function isLocalOnlyBypassableByManageScope(path: string): boolean {
  if (isStrictLoopbackOnlyPath(path)) return false;
