...
async function refreshDashboardSessionIfNeeded(
  ...
  if (!token) {
    const isLoopback = locality === "loopback" || isLoopbackRequest(request);
    if (isLoopback && !(await isAuthRequired(request, { loopback: true }))) {
...
export async function runAuthzPipeline(
  ...
  applyCorsHeaders(response, request, corsRelaxOrigin);
  if (managementDashboardRoute) {
    await refreshDashboardSessionIfNeeded(response, request, peerLocality);
  }
  return response;
}
