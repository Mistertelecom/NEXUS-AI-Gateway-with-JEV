...
export const managementPolicy: RoutePolicy = {
  ...
  async evaluate(ctx: PolicyContext): Promise<AuthOutcome> {
    ...
    // Reject off-device control requests before evaluating credentials.
    if (isStrictLoopbackOnlyPath(path) && !isLoopbackRequest(ctx)) {
      return reject(403, "LOCAL_ONLY", "NEXUS requires a trusted loopback peer");
    }
    ...
    const loopback = isLoopbackRequest(ctx);
    const privateLanAllowed = !isStrictLoopbackOnlyPath(path) && isPrivateLanRequest(ctx);
    if (isLocalOnlyPath(path, ctx.request?.method) && !loopback && !privateLanAllowed) {
