...
import { handleNexusPairComboChat } from "@/nexus/pairs/runtime";
import { isNexusWorkerStage } from "@/nexus/pairs/workerReasoning";
import { resolveComboTargets, shouldSkipConnDisable } from "@omniroute/open-sse/services/combo.ts";
      ...
      handleSingleModel: (b: Record<string, unknown>, m: string, target?: SingleModelTarget) => {
        ...
        return handleSingleModelChat(
          ...
          telemetry,
          {
            ...
            forcedConnectionId: resolvedTarget?.connectionId ?? null,
            allowedConnectionIds: resolvedTarget?.allowedConnectionIds ?? null,
            comboStepId: resolvedTarget?.stepId || null,
            comboExecutionKey: resolvedTarget?.executionKey || resolvedTarget?.stepId || null,
            ...
            preselectedCredentials: (() => {
              const key = getComboCredentialCacheKey(m, resolvedTarget ?? undefined);
            ...
            correlationId: reqId,
            conversationId,
            modelPinned: resolvedTarget?.modelPinned ?? false,
            reasoningTransportFallback,
            reasoningDecision: isNexusWorkerStage(b) ? null : reasoningDecision,
            reasoningIntent: isNexusWorkerStage(b) ? null : reasoningIntent,
          ...
          resolvedTarget?.effectiveComboStrategy ?? combo.strategy,
          true
...
async function handleSingleModelChat(
  ...
  if ((resolved as any).combo) {
    ...
    return handleNexusPairComboChat({
      body,
      combo: redirectCombo,
