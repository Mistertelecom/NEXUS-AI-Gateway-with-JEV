...
function GlobalErrorContent({ error, reset }: GlobalErrorProps) {
  ...
  return (
    <main role="alert" aria-live="assertive" className="flex flex-col items-center">
      <NexusLogo size={32} className="text-white mb-6" />
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button
          ...
          className="px-8 py-3 rounded-lg text-black bg-white hover:bg-zinc-200 text-xs font-mono font-bold cursor-pointer transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] border-none"
        >
          {t("globalError.tryAgain")}
        </button>
        <a
          href="/status"
          className="px-8 py-3 rounded-lg text-white border border-zinc-800 bg-zinc-950 hover:bg-zinc-900 text-xs font-mono font-bold no-underline transition-colors"
...
export default function GlobalError({ error, reset }: GlobalErrorProps) {
  ...
  return (
    <html lang={locale}>
      <body className="flex flex-col items-center justify-center min-h-screen p-6 bg-black text-white font-mono text-center m-0">
