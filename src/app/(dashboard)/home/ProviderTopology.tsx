...
function RouterNode({ data }: { data: RouterNodeData }) {
  return (
    <div className="flex items-center gap-2.5 px-5 py-2.5 rounded-xl border border-white/30 bg-black shadow-[0_0_20px_rgba(255,255,255,0.08)] min-w-[140px] justify-center">
      <Handle
        type="source"
        position={Position.Top}
        id="top"
      ...
      <div className="flex items-center justify-center size-7 rounded-md bg-zinc-900 border border-zinc-700 shrink-0">
        <span className="material-symbols-outlined text-white text-[16px]">hub</span>
      </div>
      <span className="text-sm font-bold text-white font-mono tracking-wider">NEXUS</span>
      {data.activeCount > 0 && (
        <span className="ml-1 px-1.5 py-0.5 rounded-full bg-white text-black text-[10px] font-mono font-bold leading-none">
          {data.activeCount}
        </span>
      )}
    </div>
