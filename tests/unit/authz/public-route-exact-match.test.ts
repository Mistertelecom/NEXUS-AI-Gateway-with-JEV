...
test("NEXUS management APIs are not public", () => {
  for (const route of ["/api/nexus/providers", "/api/nexus/doctor", "/api/nexus/workflows/runs"]) {
    assert.equal(isPublicApiRoute(route, "GET"), false, route);
    assert.equal(classifyRoute(route, "GET").routeClass, "MANAGEMENT", route);
