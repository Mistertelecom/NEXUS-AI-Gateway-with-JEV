...
const isNexusPackage = productPackage.name === "nexus-ai-control-plane";
  ...
  if (!nodeSupport.nodeCompatible) {
    ...
    console.error(
      `\x1b[31m✖ Node.js ${nodeSupport.nodeVersion} is not supported.\x1b[0m\n` +
        ...
        `  If you installed NEXUS globally, run \`node -v\` and confirm \`omniroute\` is not resolving to\n` +
...
const _notifier =
  updateNotifier && !isNexusPackage
    ? updateNotifier({ pkg: productPackage, updateCheckInterval: 1000 * 60 * 60 * 24 })
    : null;
process.on("exit", () => {
...
try {
  ...
  if (isNexusPackage) {
    program.name("nexus").description(productPackage.description);
  }
