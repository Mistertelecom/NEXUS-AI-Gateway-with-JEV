...
import {
  evaluateCliVersion,
  resolvePackBootPaths,
  resolvePackBootVersions,
} from "./pack-boot-identity.mjs";
...
/**
 * Boot verdict: HTTP 200 + a JSON body reporting the expected engine version.
...
async function main() {
  ...
  const engineManifest = JSON.parse(
    fs.readFileSync(path.join(ROOT, "open-sse", "package.json"), "utf8")
  );
  const { cliVersion, healthVersion: expectedVersion } = resolvePackBootVersions(
    manifest,
    engineManifest.version
  );
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nexus-pack-boot-"));
  let child = null;
  let tail = [];
  let exitCode = 1;
  ...
  try {
    log(`packing ${manifest.name} v${manifest.version}…`);
    ...
    const { packageRoot, binPath } = resolvePackBootPaths(prefix, manifest);
    const dataDir = path.join(tmp, "data");
    fs.mkdirSync(dataDir, { recursive: true });
    const cliOutput = execFileSync(binPath, ["--version"], {
      encoding: "utf8",
      timeout: 30_000,
      env: {
        ...process.env,
    ...
    const cliVerdict = evaluateCliVersion(cliOutput, cliVersion);
    if (!cliVerdict.ok) throw new Error(cliVerdict.failures.join("; "));
    log(`installed CLI reports ${cliVersion}`);
